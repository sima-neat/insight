...
| `POST` | `/api/mediasrc/start` | JSON `{"index": 1}` | Start one assigned source and mark it `playing`. Returns 409 for an external slot, or if the slot was reassigned, reset or unassigned while its FPS rendition was being prepared (start it again). |
