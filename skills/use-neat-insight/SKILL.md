...
- When a Viewer Configuration change appears ineffective, distinguish the global scope from a tile's channel scope. Channel-specific 3D Pose settings override global defaults until **Use Global Settings** is selected. Metadata types also have a **Show Overlay** switch; use it instead of treating style rows as visibility controls.
- **Panel Transparency** in the 3D Pose tab adds transparency to auxiliary-panel surfaces without fading the visualization or controls. The global value reaches channels that do not have a channel-specific 3D Pose override.
