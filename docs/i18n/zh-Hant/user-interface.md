...
周邊設備會列出連接至開發板的相機，並顯示每部相機所回報的模式。探索只會讀取裝置資訊，不會發佈串流，因此除了掃描讀取 MIPI 相機自身模式的短暫時間外，相機仍可供應用程式使用。**預覽**是唯一的例外，而且只會在您啟動時執行。相機匯出 API 也會從快取的掃描結果運作，而不接觸開發板。

### 選取的開發板

Insight 使用一個選取的開發板，並將其顯示在標頭中。選取它可開啟開發板設定，您可以在其中變更開發板、測試連線，或信任重新燒錄之開發板的主機金鑰。周邊設備使用此選取項目。統計資料檢視仍使用舊版的本機或 `cfg.json` 目標，尚未遵循此選取項目。
...
選取 **開始預覽** 即可查看相機畫面。開發板會擷取影片、使用硬體編碼，並將其傳送至 Insight 的檢視器；預覽會顯示在頁面中，並保留一個檢視器頻道。

預覽會占用相機，因此在停止預覽之前，您的應用程式無法開啟相機。Insight 不會在其他程序已使用的相機上開始預覽，也絕不會替您停止該程序。當您選取 **停止**、離開頁面、重新整理掃描或變更選取的開發板時，擷取就會停止。Insight 停止監看後不久也會自行停止，因此瀏覽器中斷或 Insight 重新啟動不會讓相機保持忙碌狀態。

預覽適用於 Insight 列為可用模式的 MIPI 相機。USB 相機可以被探索和匯出，但目前尚無法預覽。
