...
![ Insight 串流來源檢視畫面，其中有兩個外部插槽，其中一個被標記為使用不支援的編解碼器。](../../images/insight-external-source.png)
...
顏色會在識別對象首次出現時依頻道分配，並在頻道保持連線期間維持不變。同一頻道上的類別標籤共用一份分配，因此 `person` 在偵測、分割和分類中看起來都相同。覆寫設定依中繼資料類型分別套用：物件偵測中針對 `person` 的項目不會改變分割或分類中 `person` 的顏色。軌跡與姿勢會分別分配。當出現的識別對象多於調色盤可容納的數量時，會重複使用最久未見之識別對象的顏色，讓目前畫面上的識別對象盡可能保持區別。當畫面上的識別對象多於調色盤可容納的數量時，超出調色盤大小的顏色可能會在不同畫面之間變動。
