...
- **開發板** — 選取開發板導向功能所使用的開發板、測試 SSH 連線，並信任重新燒錄之開發板的主機金鑰。
- **周邊設備** — 探索選取開發板上的 MIPI 和 USB 相機，並透過 API 產生相機輸入設定。
- **Sentinel** — 檢查並安裝選取開發板上的 Sentinel 常駐程式、讀取其即時指標和近期歷史記錄，以及開始、停止、列出、比較和刪除追蹤。
