...
- 업로드 및 가져오기는 단일 JSON 객체 대신 `text/plain` 형식의 진행 상황 스트림을 반환합니다.
...
- **보드** — 보드 대상 기능이 사용할 보드를 선택하고, SSH 연결을 테스트하고, 다시 플래시된 보드의 호스트 키를 신뢰합니다.
- **주변 장치** — 선택한 보드에서 MIPI 및 USB 카메라를 검색하고 API를 통해 카메라 입력 구성을 렌더링합니다.
- **Sentinel** — 선택한 보드에서 Sentinel 데몬을 확인하고 설치하며, 실시간 지표와 최근 기록을 읽고, 트레이스를 시작, 중지, 나열, 비교 및 삭제합니다.
