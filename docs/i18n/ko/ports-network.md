...
Insight는 또한 `127.0.0.1:9997`(TCP)에서 mediamtx 제어 API를 실행합니다. 이 API는 루프백 전용이고, 시작할 때마다 생성되는 비밀번호로 보호되며, Insight가 외부 게시자를 감지하는 데에만 사용하므로 포트 매핑이 필요 없고 강제로 해제되지도 않습니다. 포트가 이미 사용 중이면 Insight는 API 없이 mediamtx를 시작하고 경고를 기록합니다. 스트리밍은 평소처럼 작동하며 외부 게시자 감지만 꺼집니다. 별도로 시작되어 API 비밀번호를 알고 있는 mediamtx에 연결하려면 `NEAT_INSIGHT_MEDIAMTX_API_PASS`(영문자, 숫자, `_`, `-`만 허용)를 설정하십시오.
