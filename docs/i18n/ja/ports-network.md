...
Insight は、mediamtx の制御 API も `127.0.0.1:9997`（TCP）で実行します。この API はループバック専用で、起動のたびに生成されるパスワードで保護されており、Insight 自身が外部の配信元を検出するためだけに使用します。そのため、ポートマッピングは不要で、強制的に解放されることもありません。ポートがすでに使用されている場合、Insight は API なしで mediamtx を起動し、警告をログに記録します。ストリーミングは通常どおり動作し、外部配信元の検出だけが無効になります。
