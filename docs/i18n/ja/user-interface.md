...
色は、識別対象が初めて現れたときにチャンネルごとに割り当てられ、そのチャンネルが接続されている間は固定されます。クラスラベルはチャンネル内で1つの割り当てを共有するため、`person` は検出、セグメンテーション、分類のいずれでも同じ色で表示されます。上書き設定はメタデータの種類ごとに適用されます。オブジェクト検出で `person` のエントリーを設定しても、セグメンテーションや分類の `person` の色は変わりません。トラックと姿勢は別々に割り当てられます。パレットの色数を超える識別対象が現れた場合は、最も長い間見られていない識別対象の色が再利用されるため、現在画面に表示されている識別対象はできるだけ区別しやすいまま保たれます。パレットの色数を超える識別対象が画面に表示されている場合、パレットのサイズを超える分の色はフレーム間で変わることがあります。
