...
- **ボード** — ボード向け機能が使用するボードを選択し、SSH 接続をテストし、再フラッシュされたボードのホストキーを信頼します。
- **周辺機器** — 選択したボード上の MIPI および USB カメラを検出し、API を通じてカメラ入力設定をレンダリングします。
- **Sentinel** — 選択したボード上の Sentinel デーモンを確認してインストールし、ライブメトリクスと最近の履歴を読み取り、トレースを開始、停止、一覧表示、比較、削除します。
