...
#
# Output: <output_dir>/sima-mipi-util_<version>_all.deb  (prints its path on stdout)
set -euo pipefail
...
install -d "$STAGE/usr/share/sima-mipi-util" \
           "$STAGE/usr/bin" \
...
mkdir -p "$OUT_DIR"
DEB="$OUT_DIR/sima-mipi-util_${VERSION}_all.deb"
